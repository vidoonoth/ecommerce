            ...
            <div class="p-6 bg-[#fef8f5] border-b border-gray-200">
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    @foreach ($brands as $brand)
                        <a href="{{ route('customer.products.byBrand', $brand->id) }}" class="block">
                            <div class="border border-gray-200 hover:bg-[#e4dbd1] cursor-pointer rounded-lg p-4 text-center">
                                ...
                                {{-- You can add an image or description for the brand here if available --}}
                                {{-- <img src="{{ asset('storage/' . $brand->image) }}" alt="{{ $brand->name }}" class="w-full h-40 object-cover mb-4 rounded"> --}}
                            </div>
                        </a>
                    @endforeach
                </div>
