            ...
            <div class="w-80 bg-gray-50 border-r flex flex-col h-full">
                <div class="px-6 py-6 font-bold text-xl border-b bg-white flex items-center justify-between">
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit"
                            class="text-red-500 hover:text-red-700 font-semibold text-sm px-3 py-1 rounded transition">Logout</button>
                    </form>
                </div>
                <div id="chat-user-list" class="flex-1 overflow-y-auto">
                    @foreach ($customers as $customer)
                        <div class="chat-user-item px-6 py-4 cursor-pointer hover:bg-blue-100 border-b flex items-center gap-3"
                            data-user-id="{{ $customer->id }}">
                            <div
                                class="w-10 h-10 bg-blue-200 rounded-full flex items-center justify-center font-bold text-blue-700 text-lg">
                                {{ strtoupper(substr($customer->name, 0, 2)) }}</div>
            ...
            <div class="flex-1 flex flex-col h-full">
                ...
                <form id="chat-form" class="flex border-t px-8 py-6 bg-gray-50">
                    <input type="text" id="chat-input"
                        class="flex-1 px-4 py-3 outline-none border rounded-l-lg text-base" placeholder="Ketik pesan..."
                        autocomplete="off" />
                    <button type="submit"
                        class="bg-blue-500 text-white px-8 py-3 rounded-r-lg text-base font-semibold">Kirim</button>
                </form>
            </div>
        </div>
    @endauth
